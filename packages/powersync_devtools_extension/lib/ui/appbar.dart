...
final class PowerSyncLogo extends StatefulWidget {
  ...
  @override
  State<PowerSyncLogo> createState() => _PowerSyncLogoState();
}

class _PowerSyncLogoState extends State<PowerSyncLogo> {
  late String light, dark;

  @override
  void initState() {
    super.initState();

    // Loading URI resources appears to be broken when the app gets copied as a
    // DevTools extension, so we use web object URLs from embedded strings as a
    // hacky workarond.
    light = web.URL.createObjectURL(
      web.Blob([_light.toJS].toJS, web.BlobPropertyBag(type: 'image/svg+xml')),
    );
    dark = web.URL.createObjectURL(
      web.Blob([_dark.toJS].toJS, web.BlobPropertyBag(type: 'image/svg+xml')),
    );
  }
  ...
  void dispose() {
    ...
    web.URL.revokeObjectURL(light);
    web.URL.revokeObjectURL(dark);
  }
